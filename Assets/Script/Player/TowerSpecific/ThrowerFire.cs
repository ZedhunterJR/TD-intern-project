...
public class ThrowerFire : TowerAttack
{
    ...
    protected override void OnAwake()
    {
        ...
        PoolManager.Instance.RegisterProjectilePool(proj, "Fire_thrower_bullet", 5, projKey);

        var ex = Resources.Load<GameObject>("Prefab/explosion_object");
        var explos = Instantiate(ex);
        PoolManager.Instance.RegisterProjectilePool(explos, "Fire_impact", 5, explosKey);
    }
    ...
    private void Explosion(Vector2 spot)
    {
        ...
        sc.PreDestruct = () => PoolManager.Instance.ReturnProjectileToPool(instance, projKey); 
        sc.HitEvent = (target) =>
        {
