...
public class ThrowerWater : TowerAttack
{
    protected override void OnAwake()
    {
        ...
        PoolManager.Instance.RegisterProjectilePool(proj, "Water_thrower_bullet", 5, "thrower_water_proj");

        var ex = Resources.Load<GameObject>("Prefab/explosion_object");
        var explos = Instantiate(ex);
        PoolManager.Instance.RegisterProjectilePool(explos, "Water_impact", 5, "thrower_water_explos");
    }
    protected override void Attack(GameObject target)
    {
        base.Attack(target);
        //might need pooling for projectile
        var instance = PoolManager.Instance.GetProjectileFromPool("thrower_water_proj");
    ...
    {
        var instance = PoolManager.Instance.GetProjectileFromPool("thrower_water_explos");
