...
public class ThrowerEarth : TowerAttack
{
    protected override void OnAwake()
    {
        ...
        PoolManager.Instance.RegisterProjectilePool(proj, "Earth_thrower_bullet", 5, "thrower_earth_proj");

        var ex = Resources.Load<GameObject>("Prefab/explosion_object");
        var explos = Instantiate(ex);
        PoolManager.Instance.RegisterProjectilePool(explos, "Earth_impact", 5, "thrower_earth_explos");
    }
    protected override void Attack(GameObject target)
    {
        base.Attack(target);
        //might need pooling for projectile
        var instance = PoolManager.Instance.GetProjectileFromPool("thrower_earth_proj");
    ...
    {
        var instance = PoolManager.Instance.GetProjectileFromPool("thrower_earth_explos");
